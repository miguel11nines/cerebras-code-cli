...
Options:
  ...
  -f, --forward-ports PORTS Comma-separated additional ports to forward (e.g., 3000,8000-8010)
...
Environment variables:
  ...
  CEREBRAS_CODE_FORWARD_PORTS Same as --forward-ports
EOF
    exit 0
}
...
if [[ -n "${FORWARD_PORTS:-}" ]]; then
    ...
    ok "Additional ports to forward: ${FORWARD_PORTS}"
fi
...
echo ""
if [[ -n "${FORWARD_PORTS:-}" ]]; then
    echo "  Additional ports will be forwarded: ${FORWARD_PORTS}"
fi
echo "  Add more ports: echo 'FORWARD_PORTS=3000,8000' >> ~/.cerebras-code/ports.conf"
echo ""
